...
# C = 1/10, tol = 1**-5
train_x, train_y, test_x, test_y = train_test_split(x_w_inter, y, 
                                                    test_size=0.15, 
                                                    random_state=3555)
...
model1_log = RFE(model1_log, n_features_to_select=0.85)
model1_log.fit(train_x_samp, test_x_samp)

print("Num Features: %s" % (model1_log.n_features_))
print("Selected Features: %s" % (model1_log.support_))
print("Feature Ranking: %s" % (model1_log.ranking_))
